...
namespace smt {
    ...
    */
    ...
    bool context::get_cancel_flag() {
        return false; //my change for debug
        return !m.limit().inc();
    }
...
#define SMALL_NUM_PARENTS 3
    ...
    */
    ...
    bool context::propagate() {
        TRACE("propagate", tout << "propagating... qhead:" << m_qhead << " assigned_literals_num:" << m_assigned_literals.size() << "\n";);
        while (true) {
            if (inconsistent())
                return false;
    ...
    */
    bool context::decide() {
        ...
        if (!has_split_candidate(var, is_pos)) {
            lbool phase = l_undef;
            m_case_split_queue->next_case_split(var, phase); // case-split random ?
            used_queue = true;
