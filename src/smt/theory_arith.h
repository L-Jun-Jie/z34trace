...
struct theory_arith_stats {
  unsigned m_conflicts, m_add_rows, m_pivots, m_diseq_cs, m_gomory_cuts,
      m_branches, m_gcd_tests, m_gcd_conflicts, m_patches, m_patches_succ;
  unsigned m_assert_lower, m_assert_upper, m_assert_diseq, m_core2th_eqs,
      m_core2th_diseqs;
  unsigned m_th2core_eqs, m_th2core_diseqs, m_bound_props, m_offset_eqs,
      m_fixed_eqs, m_offline_eqs;
  unsigned m_max_min;
  unsigned m_assume_eqs;
  unsigned m_gb_simplify, m_gb_superpose, m_gb_compute_basis,
      m_gb_num_processed;
...
class theory_arith : public theory, public theory_opt, private Ext {
 public:
  ...
  typedef map<rational, theory_var, obj_hash<rational>, default_eq<rational> >
      rational2var;
 ...
 protected:
  ...
  bool coeffs_enabled() const {
    return proofs_enabled() || m_bound_watch != null_bool_var;
  }
  ...
  */
  struct row_entry {
    ...
    row_entry(numeral const& c, theory_var v)
        : m_coeff(c), m_var(v), m_col_idx(0) {}
  ...
  */
  struct row {
    ...
    typename vector<row_entry>::iterator begin_entries() {
      return m_entries.begin();
    }
    typename vector<row_entry>::const_iterator begin_entries() const {
      return m_entries.begin();
    }
    typename vector<row_entry>::iterator end_entries() {
      return m_entries.end();
    }
    typename vector<row_entry>::const_iterator end_entries() const {
      return m_entries.end();
    }

    typename vector<row_entry>::iterator begin() { return m_entries.begin(); }
    typename vector<row_entry>::const_iterator begin() const {
      return m_entries.begin();
    }
    typename vector<row_entry>::iterator end() { return m_entries.end(); }
    typename vector<row_entry>::const_iterator end() const {
      return m_entries.end();
    }
  ...
  */
  struct column {
    ...
    typename svector<col_entry>::iterator begin_entries() {
      return m_entries.begin();
    }
    typename svector<col_entry>::const_iterator begin_entries() const {
      return m_entries.begin();
    }
    typename svector<col_entry>::iterator end_entries() {
      return m_entries.end();
    }
    typename svector<col_entry>::const_iterator end_entries() const {
      return m_entries.end();
    }
    typename svector<col_entry>::iterator begin() { return m_entries.begin(); }
    typename svector<col_entry>::const_iterator begin() const {
      return m_entries.begin();
    }
    typename svector<col_entry>::iterator end() { return m_entries.end(); }
    typename svector<col_entry>::const_iterator end() const {
      return m_entries.end();
    }
  ...
  enum bound_kind { B_LOWER, B_UPPER };
  ...
  class antecedents_t {
    ...
    bool empty() const { return m_eq_coeffs.empty() && m_lit_coeffs.empty(); }
  ...
  class antecedents {
   ...
   public:
    ...
    void push_eq(enode_pair const& p, numeral const& r, bool e) {
      a.push_eq(p, r, e);
    }
  ...
  class bound {
   protected:
    theory_var m_var;           // variable
    inf_numeral m_value;        // bound value
    unsigned m_bound_kind : 1;  // bound kind
    unsigned m_atom : 1;        // is atom (3 ≤ x ≤ 5 is not an atom; 3 ≤ x is an atom)

   public:
    bound(theory_var v, inf_numeral const& val, bound_kind k, bool a)
        : m_var(v), m_value(val), m_bound_kind(k), m_atom(a) {}
    virtual ~bound() = default;
    theory_var get_var() const { return m_var; }
    bound_kind get_bound_kind() const {
      return static_cast<bound_kind>(m_bound_kind);
    }
    ...
    virtual void push_justification(antecedents& antecedents,
                                    numeral const& coeff, bool proofs_enabled) {
    }
    virtual std::ostream& display(theory_arith const& th,
                                  std::ostream& out) const;
  };

  enum atom_kind { A_LOWER, A_UPPER };

  class atom : public bound {
   protected:
    bool_var m_bvar;
    inf_numeral m_k;           // bound value ?
   ...
   public:
    ...
    atom_kind get_atom_kind() const {
      return static_cast<atom_kind>(m_atom_kind);
    }
    ...
    void push_justification(antecedents& a, numeral const& coeff,
                            bool proofs_enabled) override {
      a.push_lit(literal(get_bool_var(), !m_is_true), coeff, proofs_enabled);
    }
    std::ostream& display(theory_arith const& th,
  ...
  class eq_bound : public bound {
   ...
   public:
    eq_bound(theory_var v, inf_numeral const& val, bound_kind k, enode* lhs,
             enode* rhs)
        : bound(v, val, k, false), m_lhs(lhs), m_rhs(rhs) {
  ...
  class derived_bound : public bound {
   ...
   public:
    derived_bound(theory_var v, inf_numeral const& val, bound_kind k)
        : bound(v, val, k, false) {}
    literal_vector const& lits() const { return m_lits; }
    eq_vector const& eqs() const { return m_eqs; }
    bool has_justification() const override { return true; }
    ...
    virtual void push_eq(enode_pair const& p, numeral const&) {
      m_eqs.push_back(p);
    }
    std::ostream& display(theory_arith const& th,
                          std::ostream& out) const override;
  ...
  class justified_derived_bound : public derived_bound {
   ...
   public:
    justified_derived_bound(theory_var v, inf_numeral const& val, bound_kind k)
        : derived_bound(v, val, k) {}
    bool has_justification() const override { return true; }
    void push_justification(antecedents& a, numeral const& coeff,
                            bool proofs_enabled) override;
  ...
  eq_set m_tmp_eq_set;
  void accumulate_justification(bound& b, derived_bound& target,
                                numeral const& coeff, literal_idx_set& lits,
                                eq_set& eqs);
  inf_numeral normalize_bound(theory_var v, inf_numeral const& k,
                              bound_kind kind);
  void mk_bound_from_row(theory_var v, inf_numeral const& coeff, bound_kind k,
                         row const& r);
  ...
  enum var_kind { NON_BASE, BASE, QUASI_BASE };

  struct var_data {
    unsigned m_row_id : 28;  // row owned by the variable, irrelevant if kind()
                             // == NON_BASE
    unsigned m_kind : 2;
    unsigned m_is_int : 1;
    unsigned m_nl_propagated : 1;
    var_data(bool is_int = false)
        : m_row_id(0),
          m_kind(NON_BASE),
          m_is_int(is_int),
          m_nl_propagated(false) {}
  ...
  class bound_trail {
   ...
   public:
    bound_trail(theory_var v, bound* b, bool is_upper)
        : m_var(v << 1 | static_cast<int>(is_upper)), m_old_bound(b) {}
  ...
  vector<inf_numeral>
      m_value;  // per var, the current assignment for the variable.
  vector<inf_numeral>
      m_old_value;  // per var, the old assignment for the variable.
  ptr_vector<bound> m_bounds[2];  // per var, lower bound & upper_bound
  vector<atoms> m_var_occs;       // per var, atoms that contain a variable
  svector<unsigned> m_unassigned_atoms;  // per var, the number of unassigned
                                         // atoms that contain a variable.
  ...
  ptr_vector<atom>
      m_new_atoms;  // new bound atoms that have yet to be internalized.
  svector<theory_var> m_nl_monomials;  // non linear monomials
  svector<theory_var>
      m_nl_propagated;  // non linear monomials that became linear
  v_dependency_manager
      m_dep_manager;  // for tracking bounds during non-linear reasoning

  vector<uint_set> m_row_vars;  // variables in a given row. Used during
                                // internalization to detect repeated variables.
  unsigned m_row_vars_top = 0;

  var_heap m_to_patch;   // heap containing all variables v s.t. m_value[v] does
                         // not satisfy bounds of v.
  nat_set m_left_basis;  // temporary: set of variables that already left the
                         // basis in make_feasible
  bool m_blands_rule = false;

  svector<unsigned>
      m_update_trail_stack;  // temporary trail stack used to restore the last
                             // feasible assignment.
  nat_set m_in_update_trail_stack;  // set of variables in m_update_trail_stack

  svector<unsigned>
      m_to_check;         // rows that should be checked for theory propagation
  ...
  struct var_value_eq {
    ...
    bool operator()(theory_var v1, theory_var v2) const {
      return m_th.get_value(v1) == m_th.get_value(v2) &&
             m_th.is_int_src(v1) == m_th.is_int_src(v2);
    }
  };
  ...
  bool has_var(expr* v) const {
    return get_context().e_internalized(v) &&
           get_context().get_enode(v)->get_th_var(get_id()) != null_theory_var;
  }
  theory_var expr2var(expr* v) const {
    SASSERT(get_context().e_internalized(v));
    return get_context().get_enode(v)->get_th_var(get_id());
  }
  expr* var2expr(theory_var v) const { return get_enode(v)->get_expr(); }
  bool reflection_enabled() const;
  bool reflect(app* n) const;
  unsigned lazy_pivoting_lvl() const {
    return m_params.m_arith_lazy_pivoting_lvl;
  }
  bool propagate_eqs() const {
    return m_params.m_arith_propagate_eqs &&
           m_num_conflicts < m_params.m_arith_propagation_threshold;
  }
  bool propagate_diseqs() const { return false; }
  bool random_initial_value() const {
    return m_params.m_arith_random_initial_value;
  }
  int random_lower() const { return m_params.m_arith_random_lower; }
  int random_upper() const { return m_params.m_arith_random_upper; }
  unsigned blands_rule_threshold() const {
    return m_params.m_arith_blands_rule_threshold;
  }
  bound_prop_mode propagation_mode() const {
    return m_num_conflicts < m_params.m_arith_propagation_threshold
               ? m_params.m_arith_bound_prop
               : bound_prop_mode::BP_NONE;
  }
  bool adaptive() const { return m_params.m_arith_adaptive; }
  double adaptive_assertion_threshold() const {
    return m_params.m_arith_adaptive_assertion_threshold;
  }
  unsigned max_lemma_size() const { return m_params.m_arith_max_lemma_size; }
  unsigned small_lemma_size() const {
    return m_params.m_arith_small_lemma_size;
  }
  bool relax_bounds() const { return m_params.m_arith_stronger_lemmas; }
  bool skip_big_coeffs() const {
    return m_params.m_arith_skip_rows_with_big_coeffs;
  }
  ...
  bool is_base(theory_var v) const {
    return v != null_theory_var && get_var_kind(v) == BASE;
  }
  bool is_quasi_base(theory_var v) const {
    return v != null_theory_var && get_var_kind(v) == QUASI_BASE;
  }
  bool is_non_base(theory_var v) const {
    return v != null_theory_var && get_var_kind(v) == NON_BASE;
  }
  void set_var_kind(theory_var v, var_kind k) { m_data[v].m_kind = k; }
  unsigned get_var_row(theory_var v) const {
    SASSERT(!is_non_base(v));
    return m_data[v].m_row_id;
  }
  ...
  inf_numeral const& get_quasi_base_value(theory_var v) const {
    return get_implied_value(v);
  }
  inf_numeral const& get_value(theory_var v) const {
    return is_quasi_base(v) ? get_quasi_base_value(v) : m_value[v];
  }
  bound* get_bound(theory_var v, bool upper) const {
    return m_bounds[static_cast<unsigned>(upper)][v];
  }
  bound* lower(theory_var v) const { return m_bounds[0][v]; }
  bound* upper(theory_var v) const { return m_bounds[1][v]; }
  inf_numeral const& lower_bound(theory_var v) const {
    SASSERT(lower(v) != 0);
    return lower(v)->get_value();
  }
  inf_numeral const& upper_bound(theory_var v) const {
    SASSERT(upper(v) != 0);
    return upper(v)->get_value();
  }
  bool below_lower(theory_var v) const {
    bound* l = lower(v);
    return l != nullptr && get_value(v) < l->get_value();
  }
  bool above_upper(theory_var v) const {
    bound* u = upper(v);
    return u != nullptr && get_value(v) > u->get_value();
  }
  bool below_upper(theory_var v) const {
    bound* u = upper(v);
    return u == nullptr || get_value(v) < u->get_value();
  }
  bool above_lower(theory_var v) const {
    bound* l = lower(v);
    return l == nullptr || get_value(v) > l->get_value();
  }
  ...
  bool at_upper(theory_var v) const {
    bound* u = upper(v);
    return u != nullptr && get_value(v) == u->get_value();
  }
  bool is_free(theory_var v) const {
    return lower(v) == nullptr && upper(v) == nullptr;
  }
  bool is_non_free(theory_var v) const {
    return lower(v) != nullptr || upper(v) != nullptr;
  }
  bool is_bounded(theory_var v) const {
    return lower(v) != nullptr && upper(v) != nullptr;
  }
  bool is_free(expr* n) const {
    SASSERT(get_context().e_internalized(n) &&
            get_context().get_enode(n)->get_th_var(get_id()) !=
                null_theory_var);
  ...
  void restore_bound(theory_var v, bound* new_bound, bool upper) {
  ...
  inf_numeral const& get_epsilon(theory_var v) const {
    return is_real(v) ? this->m_real_epsilon : this->m_int_epsilon;
  }
  ...
  void mk_clause(literal l1, literal l2, unsigned num_params,
                 parameter* params);
  void mk_clause(literal l1, literal l2, literal l3, unsigned num_params,
                 parameter* params);
  ...
  typename atoms::iterator next_inf(atom* a1, atom_kind kind,
                                    ...
                                    typename atoms::iterator end,
                                    bool& found_compatible);
  typename atoms::iterator first(atom_kind kind, typename atoms::iterator it,
                                 typename atoms::iterator end);
  struct compare_atoms {
    bool operator()(atom* a1, atom* a2) const {
      return a1->get_k() < a2->get_k();
    }
  };
  ...
  */
  ...
  //
  // -----------------------------------
  void add_row(unsigned r1, const numeral& coeff, unsigned r2,
               bool apply_gcd_test);
  ...
  //
  // -----------------------------------
  bool m_changed_assignment = false;  //!< auxiliary variable set to true when
  ...
  void set_value(theory_var v, const inf_numeral& new_val) {
    update_value(v, new_val - m_value[v]);
  }
  ...
  // -----------------------------------
  template <bool Lazy>
  void pivot(theory_var x_i, theory_var x_j, numeral const& a_ij,
             bool apply_gcd_test);
  template <bool Lazy>
  void eliminate(theory_var x_i, bool apply_gcd_test);
  void update_and_pivot(theory_var x_i, theory_var x_j, numeral const& a_ij,
                        inf_numeral const& x_i_new_val);
  int get_num_non_free_dep_vars(theory_var v, int best_so_far);
  theory_var select_blands_pivot_core(theory_var x_i, bool is_below,
                                      numeral& out_a_ij);
  ...
  void is_row_useful_for_bound_prop(row const& r, int& lower_idx,
                                    int& upper_idx) const;
  ...
  unsigned mk_implied_bound(row const& r, unsigned idx, bool lower,
                            theory_var v, bound_kind kind,
                            inf_numeral const& k);
  void assign_bound_literal(literal l, row const& r, unsigned idx, bool lower,
                            inf_numeral& delta);
  ...
  //
  // -----------------------------------
  bool get_freedom_interval(theory_var x_j, bool& inf_l, inf_numeral& l,
                            bool& inf_u, inf_numeral& u, numeral& m);
  ...
  bool ext_gcd_test(row const& r, numeral const& least_coeff,
                    numeral const& lcm_den, numeral const& consts);
  bool gcd_test();
  void mk_polynomial_ge(unsigned num_args, row_entry const* args,
                        rational const& k, expr_ref& result);
  bool max_min_infeasible_int_vars();
  void patch_int_infeasible_vars();
  void fix_non_base_vars();
  //        unsynch_mpq_manager m_es_num_manager; // manager for euclidean
  //        solver. struct euclidean_solver_bridge; bool
  ...
  typedef map<value_sort_pair, theory_var, value_sort_pair_hash,
              default_eq<value_sort_pair> >
      value2var;
  value2var m_fixed_var_table;

  typedef std::pair<theory_var, numeral> var_offset;
  typedef pair_hash<int_hash, obj_hash<numeral> > var_offset_hash;
  typedef map<var_offset, int, var_offset_hash, default_eq<var_offset> >
      var_offset2row_id;
  ...
  bool is_offset_row(row const& r, theory_var& x, theory_var& y,
                     numeral& k) const;
  void propagate_cheap_eq(unsigned rid);
  void propagate_eq_to_core(theory_var x, theory_var y,
                            antecedents& antecedents);
  ...
  //
  // -----------------------------------
  void set_conflict(unsigned num_literals, literal const* lits,
                    unsigned num_eqs, enode_pair const* eqs,
                    antecedents& antecedents, char const* proof_rule);
  void set_conflict(antecedents const& ante, antecedents& bounds,
                    char const* proof_rule);
  void set_conflict(derived_bound const& ante, antecedents& bounds,
                    char const* proof_rule);
  void collect_fixed_var_justifications(row const& r,
  ...
  //
  // -----------------------------------
  void push_bound_trail(theory_var v, bound* old_bound, bool is_upper) {
    m_bound_trail.push_back(bound_trail(v, old_bound, is_upper));
  }
  void push_dec_unassigned_atoms_trail(theory_var v) {
    m_unassigned_atoms_trail.push_back(v);
  }
  ...
  */
  ...
  max_min_t max_min(theory_var v, bool max, bool maintain_integrality,
                    bool& has_shared);
  bool has_interface_equality(theory_var v);
  bool max_min(svector<theory_var> const& vars);

  max_min_t max_min(row& r, bool max, bool maintain_integrality,
                    bool& has_shared);
  bool unbounded_gain(inf_numeral const& max_gain) const;
  bool safe_gain(inf_numeral const& min_gain,
                 inf_numeral const& max_gain) const;
  void normalize_gain(numeral const& divisor, inf_numeral& max_gain) const;
  void init_gains(theory_var x, bool inc, inf_numeral& min_gain,
                  inf_numeral& max_gain);
  bool update_gains(bool inc, theory_var x_i, numeral const& a_ij,
                    inf_numeral& min_gain, inf_numeral& max_gain);
  bool move_to_bound(theory_var x_i, bool inc, unsigned& best_efforts,
                     bool& has_shared);
  bool pick_var_to_leave(theory_var x_j, bool inc, numeral& a_ij,
                         inf_numeral& min_gain, inf_numeral& max_gain,
  ...
  struct var_num_occs_lt {
    bool operator()(var_num_occs const& vn1, var_num_occs const& vn2) const {
      return vn1.second > vn2.second;
    }
  };
  ...
  */
  ...
  bool is_pure_monomial(theory_var v) const {
    return is_pure_monomial(get_enode(v)->get_expr());
  }
  void mark_var(theory_var v, svector<theory_var>& vars,
                var_set& already_found);
  void mark_dependents(theory_var v, svector<theory_var>& vars,
                       var_set& already_found, row_set& already_visited_rows);
  ...
  void mk_derived_nl_bound(theory_var v, inf_numeral const& coeff, bound_kind k,
                           v_dependency* dep);
  ...
  bool get_polynomial_info(buffer<coeff_expr> const& p,
                           sbuffer<var_num_occs>& vars);
  ...
  bool in_monovariate_monomials(buffer<coeff_expr>& p, expr* var, unsigned& i1,
                                rational& c1, unsigned& n1, unsigned& i2,
                                rational& c2, unsigned& n2);
  ...
  grobner::monomial* mk_gb_monomial(rational const& coeff, expr* m, grobner& gb,
                                    v_dependency*& dep, var_set& already_found);
  void add_monomial_def_to_gb(theory_var v, grobner& gb);
  void add_row_to_gb(row const& r, grobner& gb);
  void init_grobner_var_order(svector<theory_var> const& nl_cluster,
                              grobner& gb);
  ...
  bool is_inconsistent(interval const& I, unsigned num_monomials,
                       grobner::monomial* const* monomials, v_dependency* dep);
  ...
  bool get_gb_eqs_and_look_for_conflict(ptr_vector<grobner::equation>& eqs,
                                        grobner&);
  bool scan_for_linear(ptr_vector<grobner::equation>& eqs, grobner&);
  bool try_to_modify_eqs(ptr_vector<grobner::equation>& eqs, grobner&,
                         unsigned&);
 ...
 public:
  ...
  //
  // -----------------------------------
  expr_ref mk_ge(generic_model_converter& fm, theory_var v,
                 inf_numeral const& val);
  inf_eps_rational<inf_rational> maximize(theory_var v, expr_ref& blocker,
                                          bool& has_shared) override;
  ...
  void record_conflict(unsigned num_lits, literal const* lits, unsigned num_eqs,
                       enode_pair const* eqs, unsigned num_params,
                       parameter* params);
 ...
 protected:
  ...
  void display_coeff_exprs(std::ostream& out,
                           buffer<coeff_expr> const& p) const;
...
class mi_ext {
 public:
  ...
  static numeral fractional_part(numeral const& n) { return n - floor(n); }
...
class si_ext {
 public:
  ...
  numeral m_int_epsilon;
  numeral m_real_epsilon;
  static numeral fractional_part(inf_numeral const& n) { return n - floor(n); }
  static inf_numeral mk_inf_numeral(numeral const& n, numeral const& i) {
    UNREACHABLE();
    return inf_numeral(n);
  }
...
class inf_ext {
 public:
  ...
  inf_ext()
      : m_int_epsilon(inf_rational(rational(1))),
        m_real_epsilon(inf_rational(rational(0), true)) {}
};
