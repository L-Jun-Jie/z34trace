...
namespace smt {
    ...
    */
    ...
    bool theory_arith<Ext>::internalize_atom(app * n, bool gate_ctx) {
        ...
        m_atoms.push_back(a);
        insert_bv2a(bv, a);
        TRACE("arith_internalize", tout << "succeeded... v" << v << " is_upper:" << kind << " bound(round):" << k << "\n");
        return true;
    }
...
#define ADD_ROW(_SET_COEFF_, _ADD_COEFF_)                                       \
    ...
    */
    ...
    /**
     * save the old value of pivot variable
    */
    template<typename Ext>
    ...
    */
    ...
    void theory_arith<Ext>::update_value_core(theory_var v, inf_numeral const & delta) {
        save_value(v); // save the old value
        m_value[v] += delta;
    ...
    */
    ...
    void theory_arith<Ext>::update_value(theory_var v, inf_numeral const & delta) {
        update_value_core(v, delta); // m_value[v] += delta (v is non-base variable)

        column & c = m_columns[v]; // pivot column
        c.compress_if_needed(m_rows);

        inf_numeral delta2;
        for (auto& ce : c) {
            if (!ce.is_dead()) { // the row corresponding to col_entry can still be updated
                row & r      = m_rows[ce.m_row_id];
                theory_var s = r.get_base_var();
                if (s != null_theory_var && !is_quasi_base(s)) {
                    delta2   = delta;
                    delta2  *= r[ce.m_row_idx].m_coeff; // the effect of a change in v on the base variable s
                    delta2.neg();
                    update_value_core(s, delta2); // save old value and change the value
                }
            }
        }
    }
    ...
    */
    ...
    void theory_arith<Ext>::pivot(theory_var x_i, theory_var x_j, numeral const & a_ij, bool apply_gcd_test) {
        TRACE("arith_pivoting", tout << "pivoting: base v" << x_i << ", quasi_base v" << x_j << "\n";);
        m_stats.m_pivots++;
...
#define DIVIDE_ROW(_ADJUST_COEFF_)                                      \
            ...
            if (!it->is_dead()) {
                ...
                if (it->m_row_id != static_cast<int>(r_id)) {
                    ...
                    if (s2 != null_theory_var && (!Lazy || is_base(s2))) {
                        ...
                        a_kj.neg();
                        add_row(it->m_row_id, a_kj, r_id, apply_gcd_test); // Gaussian elimination
    ...
    */
    ...
    void theory_arith<Ext>::update_and_pivot(theory_var x_i, theory_var x_j, numeral const & a_ij, inf_numeral const & x_i_new_val) {
        ...
        SASSERT(is_base(x_i));
        inf_numeral theta = m_value[x_i]; // current assignment of x_i 
        TRACE("update_and_pivot_bug", tout << "theta 1) " << theta << "; x_i_new_val: " << x_i_new_val << "\n";);
        theta -= x_i_new_val; // current val minus target val (bound)
        TRACE("update_and_pivot_bug", tout << "theta 2) " << theta << "; a_ij: " << a_ij << "\n";);
        theta /= a_ij; // amount of change divided by pivot element
    ...
    */
    ...
    theory_var theory_arith<Ext>::select_pivot(theory_var x_i, bool is_below, numeral & out_a_ij) {
        TRACE("select_pivot", tout << "m_blands_rule: " << m_blands_rule << " pivot_row: v" << x_i << "\n";);
    ...
    */
    ...
    bool theory_arith<Ext>::make_var_feasible(theory_var x_i) {
        ...
        numeral a_ij; // pivot element
        theory_var x_j = select_pivot(x_i, is_below, a_ij); // pivot column (bland's rule ?)
    ...
    */
    ...
    bool theory_arith<Ext>::assert_upper(bound * b) {
        ...
        theory_var          v = b->get_var(); // row atoms
        inf_numeral const & k = b->get_value(); // new upper bound value

        TRACE("arith", display_bound(tout, b); tout << "v" << v << " <= " << k << "\n";);
        bound * u = upper(v); // old upper bound
        bound * l = lower(v); // old lower bound

        if (l && k < l->get_value()) { // lower bound greater than new upper bound
            sign_bound_conflict(l, b); // conflict
            return false;
        }

        if (u && k >= u->get_value()) { // old upper bound implies the new upper bound
            // redundant
            return true;
        }
        ...
        case QUASI_BASE:
            ...
            SASSERT(get_var_kind(v) == BASE);
        case BASE:
            if (!m_to_patch.contains(v) && get_value(v) > k) { // The value of v violates the upper bound
    ...
    /* assert bound like "atom <= upper_bound" or "atom >= lower_bound" */
    template<typename Ext>
...
#define UPDATE_IDX(IDX) IDX = IDX == -1 ? i : -2
    ...
    */
    ...
    unsigned theory_arith<Ext>::mk_implied_bound(row const & r, unsigned idx, bool is_lower, theory_var v, bound_kind kind, inf_numeral const & k) {
        ...
        inf_numeral const & epsilon      = get_epsilon(v); // int: 1, real: 0
        inf_numeral delta;
        TRACE("propagate_bounds", tout << "epsilon: " << epsilon << "\n";);
        unsigned count = 0;
        for (atom* a : as) {
            bool_var bv = a->get_bool_var();
            literal  l(bv);
            if (ctx.get_assignment(bv) == l_undef) { // boolean variable not yet assigned (theoretical atom)
                inf_numeral const & k2 = a->get_k(); // atom's bound
                delta.reset();
